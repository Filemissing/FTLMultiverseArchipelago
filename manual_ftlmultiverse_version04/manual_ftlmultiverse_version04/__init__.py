...
def launch_FTL_client(*args):
    import CommonClient
    from .FTLClient import launch as Main

    if CommonClient.gui_enabled:
        launch_subprocess(Main, name="FTL client")
    else:
        Main()
...
def add_client_to_launcher() -> None:
    ...
    manualClientFound = False
    FTLClientFound = False # automated client
    for c in components:
        if c.display_name == "Manual Client":
            manualClientFound = True
            if getattr(c, "version", 0) < version:  # We have a newer version of the Manual Client than the one the last apworld added
                ...
                return
        if c.display_name == "FTL Client":
            manualClientFound = True
            if getattr(c, "version", 0) < version:  # We have a newer version of the Manual Client than the one the last apworld added
                c.version = version
                c.func = launch_FTL_client
                return
    if not manualClientFound:
        components.append(VersionedComponent("Manual Client", "ManualClient", func=launch_manual_client, version=version, file_identifier=SuffixIdentifier('.apmanual')))
    if not FTLClientFound:
        components.append(VersionedComponent("FTL Client", "FTLClient", func=launch_FTL_client, version=version, file_identifier=SuffixIdentifier('.apmanual')))
